...
function registriere(app, deps) {
    ...
    app.get('/api/ticker/chats', requireUser, async (req, res) => {
        ...
        try {
            ...
            if (!antwort.ok) {
                // Die Begruendung des Gateways durchreichen statt nur den
                // Statuscode. 409 heisst dort "Sitzung ist nicht bereit
                // (starting)" - das sagt einem, was zu tun ist; "HTTP 409"
                // sagt gar nichts.
                const d = await antwort.json().catch(function () { return {}; });
                throw new Error(d.error || ('HTTP ' + antwort.status));
            }
